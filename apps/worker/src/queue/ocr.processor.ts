...
import { Contract, ContractText } from '@expirationreminderai/database';
...
export class OcrProcessor extends WorkerHost {
    ...
    async process(job: Job<{ contractId: string; fileName: string }>): Promise<any> {
        ...
        try {
            ...
            // Extract text (auto-detects PDF vs DOC vs DOCX by extension)
            const fullText = await this.ocrService.extractText(tempPath);
            ...
            console.log(`[BullMQ OCR] Text extracted for contract ${contractId}, enqueuing extraction...`);

            // Chain to extraction step
            await this.extractQueue.add('extract-clauses', {
                contractId,
            });

            console.log(`[BullMQ OCR] Extraction job enqueued for contract ${contractId}`);
